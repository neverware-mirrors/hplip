...
class FaxSendJobForm(FaxSendJobForm_base):
    ...
    def __init__(self, sock, device_uri, printer_name, 
                 username, job_id, title, 
                 fax_file, standalone, job_size,
        ...
        self.MIME_TYPES_DESC = \
        {
            ...
            "application/hplip-fax" : self.__tr("HP Fax"),
            "application/hplip-fax-coverpage" : self.__tr("HP Fax Coverpage"),
        }
    ...
    def InitialUpdate(self):
        ...
        # Process any command line pre-rendered faxes
        # in job mode (--job)
        if not self.standalone and self.title and self.job_id:
            log.debug("Job Mode: Adding pre-rendered file: %s (%d)" % (self.title, self.job_id))
            QApplication.setOverrideCursor(QApplication.waitCursor)
            self.addFileFromJob(EVENT_FAX_RENDER_COMPLETE, self.title, self.username, self.job_id, self.job_size)
        else:
    ...
    def addFilePushButton_clicked(self):
        self.processFile(self.filename, self.title)
        self.CheckSendButtons()
...
##        self.titleEdit.setText(self.__tr("(file %1)").arg(self.document_num))
...
##        self.enableAddFileButton()
    ...
    def addFileFromJob(self, event, title, username, job_id=0, job_size=0):
        ...
        #if not title:
        #    title = self.__tr("(untitled %1)").arg(self.document_num)
